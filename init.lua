...
-- Enable followwrap in diffopt to make diff mode respect wrap setting
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      -- mason-lspconfig v2 dropped the `handlers` mechanism: it now enables every
      -- installed server automatically (automatic_enable). So we register each
      -- server's config via `vim.lsp.config` instead. The `*` entry broadcasts the
      -- blink.cmp capabilities to every server (this is what wires up completion).
